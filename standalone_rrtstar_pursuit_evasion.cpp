...
main() {
    ...
    sampler_t sampler1, sampler2, sampler3;
    distance_evaluator_t distance_evaluator1, distance_evaluator2, distance_evaluator3;
    extender_t extender1, extender2, extender3;
    collision_checker_t collision_checker1, collision_checker2, collision_checker3;
    min_time_reachability_t min_time_reachability1, min_time_reachability2, min_time_reachability3;
    ...
    rrtstar_t planner3(sampler3, distance_evaluator3, extender3, collision_checker3,
            min_time_reachability3, min_time_reachability3);
    ...
    planner3.parameters.set_phase(2); // See the documentation of the RRG algorithm for more information.
    ...
    planner3.parameters.set_gamma(35.0);
    ...
    planner3.parameters.set_dimension(3);
    planner3.parameters.set_max_radius(20.0); //   solution quickly, while preserving the asymptotic optimality.
    ...
    sampler3.set_support(sampler_support);
    ...
    region<2> obstacle_new, obstacle1, obstacle2, obstacle3, obstacle4, obstacle5, obstacle6;

    obstacle_new.center[0] = -5.0;
    obstacle_new.center[1] = 0.0;
    obstacle_new.size[0] = 2.0;
    obstacle_new.size[1] = 5.0;
    obstacle1.center[0] = 2.0;
    obstacle1.center[1] = -8.0;
    obstacle1.size[0] = 1.0;
    obstacle1.size[1] = 4.0;
    obstacle2.center[0] = 5.0;
    obstacle2.center[1] = -5.0;
    obstacle2.size[0] = 4.0;
    obstacle2.size[1] = 2.0;
    obstacle3.center[0] = -8.0;
    obstacle3.center[1] = -3.0;
    obstacle3.size[0] = 1.0;
    obstacle3.size[1] = 3.0;
    obstacle4.center[0] = 5.0;
    obstacle4.center[1] = 0.0;
    obstacle4.size[0] = 5.0;
    obstacle4.size[1] = 1.0;
    obstacle5.center[0] = 6.0;
    obstacle5.center[1] = 6.0;
    obstacle5.size[0] = 3.0;
    obstacle5.size[1] = 1.0;

    collision_checker1.add_obstacle(obstacle_new);
    collision_checker2.add_obstacle(obstacle_new);
    collision_checker3.add_obstacle(obstacle_new);

    collision_checker1.add_obstacle(obstacle1);
    collision_checker2.add_obstacle(obstacle1);
    collision_checker3.add_obstacle(obstacle1);

    collision_checker1.add_obstacle(obstacle2);
    collision_checker2.add_obstacle(obstacle2);
    collision_checker3.add_obstacle(obstacle2);

    collision_checker1.add_obstacle(obstacle3);
    collision_checker2.add_obstacle(obstacle3);
    collision_checker3.add_obstacle(obstacle3);

    collision_checker1.add_obstacle(obstacle4);
    collision_checker2.add_obstacle(obstacle4);
    collision_checker3.add_obstacle(obstacle4);

    collision_checker1.add_obstacle(obstacle5);
    collision_checker2.add_obstacle(obstacle5);
    collision_checker3.add_obstacle(obstacle5);


    obstacles << obstacle_new.center[0] << "," << obstacle_new.center[1] << "," << obstacle_new.size[0] << "," << obstacle_new.size[1] << "\n";
    obstacles << obstacle1.center[0] << "," << obstacle1.center[1] << "," << obstacle1.size[0] << "," << obstacle1.size[1] << "\n";
    obstacles << obstacle2.center[0] << "," << obstacle2.center[1] << "," << obstacle2.size[0] << "," << obstacle2.size[1] << "\n";
    obstacles << obstacle3.center[0] << "," << obstacle3.center[1] << "," << obstacle3.size[0] << "," << obstacle3.size[1] << "\n";
    obstacles << obstacle4.center[0] << "," << obstacle4.center[1] << "," << obstacle4.size[0] << "," << obstacle4.size[1] << "\n";
    obstacles << obstacle5.center[0] << "," << obstacle5.center[1] << "," << obstacle5.size[0] << "," << obstacle5.size[1] << "\n";
    // 2.d add landmarks
    ...
    for (int i = 0; i < 2; i++) {
        landmark_new1.center[i] = 0.0;
        landmark_new1.size[i] = 2.0;
    }
    landmark_new1.center[1] = 8.0;
    ...
    collision_checker3.add_landmark(landmark_new1);
    collision_checker3.add_landmark(landmark_new2);
    ...
    region<2> region_goal3;
    region_goal3.center[0] = -5.0;
    region_goal3.center[1] = 5.0;
    region_goal3.size[0] = 2.0;
    region_goal3.size[1] = 2.0;
    min_time_reachability1.set_goal_region(region_goal1);
    min_time_reachability2.set_goal_region(region_goal2);
    min_time_reachability3.set_goal_region(region_goal3);


    // 2.f Initialize the planner1
    state_t *state_initial1 = new state_t;
    state_t *state_initial2 = new state_t;
    state_t *state_initial3 = new state_t;

    state_initial1->state_vars[0] = 0.0;
    state_initial1->state_vars[1] = -9.0;
    state_initial1->state_vars[2] = 0.0;
    state_initial2->state_vars[0] = 8.0;
    state_initial2->state_vars[1] = -4.0;
    state_initial2->state_vars[2] = 0.0;
    state_initial3->state_vars[0] = -9.0;
    state_initial3->state_vars[1] = 0.0;
    state_initial3->state_vars[2] = 0.0;
    ...
    planner3.initialize(state_initial3);
    ...
    // 3. RUN THE PLANNER
    double visible_range = 5, distance_range = 7;
    vertex_t *z_e_new, *z_p_new, *z_p2_new;
    for (int i = 0; i < 5000; i++) {
        ...
        if (z_e_new != NULL) {
            //nearCapture (planner1, distance_evaluator1,z_e_new);
            //cout<<z_e_new->state->operator [](1)<<"\n";            
            list<void*> list_vertices_in_ball;
            double radius = planner2.parameters.get_gamma() * pow(log(planner2.get_num_vertices()) / planner2.get_num_vertices(), 1.0 / ((double) (planner2.parameters.get_dimension())));

            distance_evaluator2.find_near_vertices_r(z_e_new->state, visible_range, &list_vertices_in_ball);
            for (typename list<void*>::iterator iter = list_vertices_in_ball.begin(); iter != list_vertices_in_ball.end(); iter++) {
                vertex_t *vertex_curr = (vertex_t*) (*iter);
                //if( vertex_curr->data.visibility_cost>z_e_new->data.visibility_cost)
                if (planner1.isVisible(z_e_new->state, vertex_curr->state) == 1) {
                    z_e_new->state->operator[](0) = state_initial1->operator[](0);
                    z_e_new->state->operator[](1) = state_initial1->operator[](1);
                    planner1.updateCost(z_e_new, 5, 50);


                }
            }
            list_vertices_in_ball.clear();
            distance_evaluator3.find_near_vertices_r(z_e_new->state, visible_range, &list_vertices_in_ball);
            for (typename list<void*>::iterator iter = list_vertices_in_ball.begin(); iter != list_vertices_in_ball.end(); iter++) {
                vertex_t *vertex_curr = (vertex_t*) (*iter);
                //if( vertex_curr->data.visibility_cost>z_e_new->data.visibility_cost)
                if (planner1.isVisible(z_e_new->state, vertex_curr->state) == 1) {
        ...
        // planner1.collision_checker.isVisible(z_e_new->state,z_p_new->state);
        //planner2.iteration_additive_cost(&z_p_new);
        planner2.iteration_evader(&z_p_new);

        if (z_p_new != NULL && z_p_new->data.total_cost < distance_range) {
            //nearCapture (planner1, distance_evaluator1,z_e_new);
            //cout<<z_e_new->state->operator [](1)<<"\n";            
            list<void*> list_vertices_in_ball;
            double radius = planner1.parameters.get_gamma() * pow(log(planner1.get_num_vertices()) / planner1.get_num_vertices(), 1.0 / ((double) (planner1.parameters.get_dimension())));

            distance_evaluator1.find_near_vertices_r(z_p_new->state, visible_range, &list_vertices_in_ball);
            for (typename list<void*>::iterator iter = list_vertices_in_ball.begin(); iter != list_vertices_in_ball.end(); iter++) {
                vertex_t *vertex_curr = (vertex_t*) (*iter);
                //if(vertex_curr->data.visibility_cost>z_p_new->data.visibility_cost)
                if (planner2.isVisible(vertex_curr->state, z_p_new->state) == 1) {
                    vertex_curr->state->operator[](0) = state_initial1->operator[](0);
                    vertex_curr->state->operator[](1) = state_initial1->operator[](1);
                    planner1.updateCost(vertex_curr, 5, 50);
                }
            ...
            z_p_new->state->operator[](0) = state_initial2->operator[](0);
            z_p_new->state->operator[](1) = state_initial2->operator[](1);
        }
        planner3.iteration_evader(&z_p2_new);

        if (z_p2_new != NULL && z_p2_new->data.total_cost < distance_range) {
            //nearCapture (planner1, distance_evaluator1,z_e_new);
            //cout<<z_e_new->state->operator [](1)<<"\n";            
            list<void*> list_vertices_in_ball;
            double radius = planner1.parameters.get_gamma() * pow(log(planner1.get_num_vertices()) / planner1.get_num_vertices(), 1.0 / ((double) (planner1.parameters.get_dimension())));

            distance_evaluator1.find_near_vertices_r(z_p2_new->state, visible_range, &list_vertices_in_ball);
            for (typename list<void*>::iterator iter = list_vertices_in_ball.begin(); iter != list_vertices_in_ball.end(); iter++) {
                vertex_t *vertex_curr = (vertex_t*) (*iter);
                //if(vertex_curr->data.visibility_cost>z_p_new->data.visibility_cost)
                if (planner3.isVisible(vertex_curr->state, z_p2_new->state) == 1) {
                    vertex_curr->state->operator[](0) = state_initial1->operator[](0);
                    vertex_curr->state->operator[](1) = state_initial1->operator[](1);
                    planner1.updateCost(vertex_curr, 5, 50);
                }
            ...
            z_p2_new->state->operator[](0) = state_initial3->operator[](0);
            z_p2_new->state->operator[](1) = state_initial3->operator[](1);
        }
    }
    ...
    ofstream tree1, tree2, tree3, trajectory1, trajectory2, trajectory3;
    tree1.open("tree1.txt");
    tree2.open("tree2.txt");
    tree3.open("tree3.txt");
    trajectory1.open("trajectory1.txt");
    trajectory2.open("trajectory2.txt");
    trajectory3.open("trajectory3.txt");
    ...
    for (typename std::list<vertex_t*>::iterator iter = planner3.list_vertices.begin(); iter != planner3.list_vertices.end(); iter++) {//vertex_t *vertex_curr = (vertex_t*)(*iter);
        //SingleIntegrator::State *astate=*((*iter)->getState());
        double x = ((*iter)->state)->operator[](0);
        double y = ((*iter)->state)->operator[](1);
        double z = ((*iter)->state)->operator[](2);
